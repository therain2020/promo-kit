...
description: |
  ...
  支持中文（小红书/小黑盒）和英文（Twitter/LinkedIn/Reddit/Dev.to）。
...
if [ "{lang}" = "zh" ]; then
  TEMPLATE_FILE="$REPO_ROOT/references/copywriter-cn.md"
else
  TEMPLATE_FILE="$REPO_ROOT/references/copywriter-en.md"
fi
```
...
# 根据语言选择 humanizer
if [ "{lang}" = "zh" ]; then
  # 调用 Skill("humanizer-zh", args="文件路径")
else
  # 调用 Skill("humanizer", args="文件路径")
fi
```
...
1. 标题长度 — 小红书 ≤20 字，Twitter tweet 1 ≤280 字符
2. 前三秒吸引力 — 首句能否制造好奇或紧迫感
3. 真实感 — 读起来像开发者分享，不是产品说明书
4. 技术准确性 — 所有声称可验证，不夸大
5. CTA 存在 — 明确的下一步行动
6. 标签数量 — 小红书 8-10，Twitter 2-3，LinkedIn 3-5，Reddit 0
7. 配图标记 — 如需要配图，标明位置和类型
